...
def attach_fp32_hooks(model):
    ...
    for l_idx, layer in enumerate([model.layer1, model.layer2, model.layer3, model.layer4], 1):
        for b_idx, block in enumerate(layer):
            ...
            handles.append(block.relu1.register_forward_hook(get_fp32_hook(f"{pfx}_conv1_relu")))
            ...
            handles.append(block.relu2.register_forward_hook(get_fp32_hook(f"{pfx}_out")))
...
def evaluate_error(dataset_name: str, num_data: int = None, batch_size: int = 64):
    ...
    # Load dataloader
    train_mod.train_loader = None; train_mod.val_loader = None; train_mod.test_loader = None
    ...
    for batch_idx, (images, labels) in enumerate(loader, 1):
        ...
        for layer_idx in range(1, 5):
            for block_idx in range(2):
                ...
                q_added = int8_utils.integer_add(q_out2, z_out2, s_out2, q_short, z_short, s_short, z_final, s_final)
    ...
    # Finalize Metrics
    final_metrics = {name: tracker.finalize() for name, tracker in layer_trackers.items()}
...
def plot_error_metrics(metrics_dict, dataset_name):
    ...
    fig.suptitle(f'ResNet18 FP32 vs INT8 Quantization Error Progression ({dataset_name})', fontsize=16, fontweight='bold')
