"""Benchmark float vs integer inference across all ResNet datasets.

This script evaluates the same trained models used by inference.py over the
deterministic 10% test splits created in resnet18.py. It supports:
...
the correct train_data flag in resnet18.py, then benchmarks it.
"""
...
def _train_dataset_for_checkpoint(dataset_name: str, multi_trained: bool) -> bool:
	"""Train missing checkpoint with the correct resnet18 train_data selection."""
...
def _integer_accuracy(model: torch.nn.Module, loader: DataLoader, dataset_name: str = "dataset", num_data: Optional[int] = None) -> float:
	...
	for batch_idx, (images, labels) in enumerate(loader, 1):
		...
		# Quantize input using conv1 input range
		...
		# Integer forward pass (mirrors inference.main)
		...
		fc_in_range = inference.activation_ranges["fc"]
		pseudo_fc_in = torch.tensor(
			[fc_in_range["in_min"], fc_in_range["in_max"]], dtype=torch.float32
		)
		s_fc_in, z_fc_in = inference.get_quantization_params(pseudo_fc_in, num_bits=8)

		q_pooled = inference.integer_global_avg_pool2d(
...
def benchmark(dataset_names=None, num_data: Optional[int] = None) -> dict:
	...
	for name in targets:
		...
		print(f"[bench] {name}: float={float_acc:.2f}% | integer={int_acc:.2f}%")
