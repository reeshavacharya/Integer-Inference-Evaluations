        ...
        for layer_idx in range(1, 5):
            for block_idx in range(2):
                ...
                if activation == "relu":
                    ...
                    f_act = F.gelu(f_accum)
                    f_act = torch.clamp(f_act, min=-10.0, max=10.0)
                    q_x = int8_utils.quantize_tensor(
                        f_act, act_s, act_z, dtype=torch.uint8
                    )
                elif activation == "leaky_relu":
                    f_accum = int8_utils.dequantize_tensor(q_added, conv_s, conv_z)
                    f_act = F.leaky_relu(f_accum, negative_slope=1.0)
                ...
                _evaluate_step(
                    q_x, s_out, z_out, _activation_label(f"{prefix}_out", activation)
                )
...
def plot_error_metrics(metrics_dict, dataset_name, activation: str = "relu"):
    ...
    Saves the figure to `error/graphs/`.
    """
    ...
    axs[0, 0].plot(
        x, median_err, marker="v", color="orange", label="Median Absolute Error"
    )
    axs[0, 0].plot(
        x,
        std_err,
        ...
        color="magenta",
        linestyle="-.",
        label="Std Deviation (Noise Spread)",
    )
    axs[0, 0].plot(
        x,
        max_err,
    ...
    filename = os.path.join(
        graphs_dir, f"quantization_divergence_{dataset_name.lower()}_{activation}.png"
    )
...
if __name__ == "__main__":
    ...
    file_name = os.path.join(
        data_dir, f"error_accumulation_{args.dataset.lower()}_{args.activation}.json"
    )
