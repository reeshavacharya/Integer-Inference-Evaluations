...
def _load_model_module():
	from inference import _resolve_infer_config, load_calibration_ranges, activation_ranges
...
def _resolve_quantize_target(quantize_arg: str):
	...
	explicit_paths = [candidate, os.path.join(LENET_DIR, candidate), os.path.join(THIS_DIR, candidate)]
...
def process_conv(conv, bn, layer_name, scale_in):
	if bn is not None:
		...
		b_float = conv.bias.detach() if conv.bias is not None else torch.zeros(conv.out_channels, device=w_float.device)
...
def main(quantize_arg: str):
	...
	out_path = os.path.join(LENET_DIR, os.path.basename(model_path).replace(".pth", "_int8.pth"))
