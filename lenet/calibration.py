"""Offline calibration for LeNet INT8 export.

This mirrors the ResNet flow:
- select one dataset or calibrate all supported datasets
- build the deterministic test loader
- run floating-point forward passes with hooks
- save per-layer min/max and quantization parameters to JSON
"""
...
def _get_layer_config(model: nn.Module):
		...
		def callback(module, inputs, output):
			...
			if name not in activation_ranges:
				activation_ranges[name] = {
					"in_min": float(in_tensor.min().item()),
					"in_max": float(in_tensor.max().item()),
					"out_min": float(out_tensor.min().item()),
					"out_max": float(out_tensor.max().item()),
				}
				return

			current = activation_ranges[name]
			current["in_min"] = min(current["in_min"], float(in_tensor.min().item()))
			current["in_max"] = max(current["in_max"], float(in_tensor.max().item()))
			current["out_min"] = min(current["out_min"], float(out_tensor.min().item()))
			current["out_max"] = max(current["out_max"], float(out_tensor.max().item()))
...
if __name__ == "__main__":
	...
	parser.add_argument(
		...
		dest="nih_chest",
		action="store_true",
		help="Calibrate NIH-CHEST",
	)
	parser.add_argument(
		"--OCTMNIST",
		dest="octmnist",
		action="store_true",
		help="Calibrate OCTMNIST",
	)
	parser.add_argument(
		"--BloodMNIST",
		dest="bloodmnist",
		action="store_true",
		help="Calibrate BloodMNIST",
	)
	parser.add_argument(
		"--OrganAMNIST",
		dest="organamnist",
	...
	parser.add_argument(
		...
		type=str,
		default=None,
		help="Optional output directory for calibration JSON",
	)
