...
def evaluate_error(dataset_name: str, num_data: int = None, batch_size: int = 64):
	...
	state = torch.load(os.path.join(THIS_DIR, os.path.basename(cfg["model_path"])), map_location="cpu")
	if len(state) > 0 and list(state.keys())[0].startswith("module."):
		state = {key[7:]: value for key, value in state.items()}
	model.load_state_dict(state)
	model.eval()

	int8_path = os.path.join(THIS_DIR, os.path.basename(cfg["model_path"]).replace(".pth", "_int8.pth"))
