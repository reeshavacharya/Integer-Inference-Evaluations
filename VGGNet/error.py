...
int8_inference = _load_module("vgg_int8_inference", os.path.join(INT8_DIR, "inference.py"), INT8_DIR)
calibration_mod = _load_module("vgg_calibration", os.path.join(THIS_DIR, "calibration.py"), THIS_DIR)
export_mod = _load_module("vgg_int8_export", os.path.join(INT8_DIR, "export_int8_model.py"), INT8_DIR)
...
class LayerMetrics:
	...
	def update(self, fp_tensor, dq_tensor):
		...
		sqnr = (10 * torch.log10(signal_power / noise_power)).item() if noise_power > 1e-7 else 100.0
...
def _calibration_file_path(dataset_display: str):
	return os.path.join(THIS_DIR, "calibration", _calibration_file_name(dataset_display))
...
def _prepare_artifacts(cfg, batch_size: int):
	...
	if not os.path.exists(int8_path):
		print(f"[error] Missing INT8 checkpoint for {dataset_display}; exporting it now.")
...
def _resolve_test_loader(setup_fn, batch_size: int, dataset_name: str):
	...
	if train_mod.test_loader is not None:
		train_mod.validate_loader_preprocessing(train_mod.test_loader, dataset_name, stage="error")
...
def _run_int8_trace(model, images, int8_state):
	...
	for conv_idx in CONV_INDICES:
		...
		if conv_idx in POOL_AFTER_CONV:
			...
			traces[f"features_{conv_idx}_pool"] = s_out * (q_x.to(torch.float32) - z_out)

	fc_in_scale = int8_state["classifier_0"]["scale_in"]
	fc_in_zp = int8_state["classifier_0"]["zp_in"]
	q_x = int8_utils.integer_adaptive_avg_pool(q_x, z_out, s_out, fc_in_zp, fc_in_scale, output_size=(7, 7))
...
def evaluate_error(dataset_name: str, num_data: int = None, batch_size: int = 64):
	...
	train_mod.datasetDownloader(dataset_display)
	loader = _resolve_test_loader(cfg["setup_fn"], batch_size=batch_size, dataset_name=dataset_display)
...
def plot_error_metrics(metrics_dict, dataset_name):
	...
	axs[0, 0].plot(x, max_err, marker="x", color="red", linestyle="--", label="Max Absolute Error")
	...
	axs[1, 1].bar(x, mean_shift, color=["red" if value < 0 else "blue" for value in mean_shift], alpha=0.6)
...
if __name__ == "__main__":
	...
	parser.add_argument("--MNIST", dest="mnist", action="store_true", help="Evaluate MNIST")
	...
	for dataset_name in selected:
		...
		metrics = evaluate_error(dataset_name, args.num_data, batch_size=args.batch_size)
