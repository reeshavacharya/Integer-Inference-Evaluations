...
OUT_DIR_BASE = os.path.join(THIS_DIR, "graphs")
...
def main():
    ...
    fig.suptitle(f"VGG19 Activation Comparison: ReLU vs GELU vs Leaky ReLU ({ds})", fontsize=14, fontweight="bold")
    ...
    out_dir_ds = os.path.join(OUT_DIR_BASE, slug)
    os.makedirs(out_dir_ds, exist_ok=True)
    out = os.path.join(out_dir_ds, f"error_comparison_{slug}_{mode}.png")
