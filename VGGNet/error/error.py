...
class LayerMetrics:
    ...
    def update(self, fp_tensor, dq_tensor, q_tensor, scale=None, zp=None):
        ...
        # --- Strictly Quantized Negative Distribution (%) ---
        ...
        if q_tensor.dtype == torch.uint32:
            q_flat = torch.where(q_flat >= 2147483648, q_flat - 4294967296, q_flat)
...
def attach_fp32_hooks(model, activation: str):
    ...
    for conv_idx in CONV_INDICES:
        handles.append(model.features[conv_idx+1].register_forward_hook(get_fp32_hook(f"features_{conv_idx}_pre_act")))
        handles.append(model.features[conv_idx+2].register_forward_hook(get_fp32_hook(_activation_label(f"features_{conv_idx}", activation))))
        
    for fc_idx in FC_INDICES:
        if fc_idx != FC_INDICES[-1]:
            handles.append(model.classifier[fc_idx].register_forward_hook(get_fp32_hook(f"classifier_{fc_idx}_pre_act")))
            handles.append(model.classifier[fc_idx+1].register_forward_hook(get_fp32_hook(_activation_label(f"classifier_{fc_idx}", activation))))
        else:
            handles.append(model.classifier[fc_idx].register_forward_hook(get_fp32_hook(f"classifier_{fc_idx}")))
    return handles
...
def evaluate_error(dataset_name: str, num_data: int = None, batch_size: int = 64, activation: str = "relu", mode: str = "int8"):
    ...
    target_dtype = torch.uint32 if mode == "int32" else torch.uint8
    quant_utils = int32_utils if mode == "int32" else int8_utils
    infer_module = int32_inference if mode == "int32" else int8_inference
    ...
    with torch.no_grad():
        for batch_idx, (images, _labels) in enumerate(loader, 1):
            ...
            def _evaluate_step(q_tensor, s_out, z_out, name):
                ...
                q_eval = q_tensor.to(torch.float64)
                if q_tensor.dtype == torch.uint32:
                    # Unsigned Integer Underflow correction
                    # Reconstructs signed integer semantics mathematically for values >= 2^31
                    q_eval = torch.where(q_eval >= 2147483648.0, q_eval - 4294967296.0, q_eval)
                
                dq_tensor = s_out * (q_eval - z_out)
                layer_trackers[name].update(fp32_tensors[name], dq_tensor, q_tensor, s_out, z_out)

            scale_in = quant_state["meta"]["in_scale"]
            zp_in = quant_state["meta"]["in_zp"]
            ...
            for conv_idx in CONV_INDICES:
                ...
                layer_data = quant_state[layer_name]
                if mode == "int32" and activation == "gelu":
                    layer_data["gelu_lut"] = quant_state[f"{layer_name}_gelu_lut"]

                if mode == "int32":
                    q_pre_act, s_out, z_out = infer_module.run_integer_layer(
                        q_x, layer_data, z_out, layer_type="conv",
                        apply_relu=False, apply_maxpool=False, activation=activation
                    )
                else:
                    q_pre_act, s_out, z_out = infer_module.run_integer_layer(
                        q_x, layer_data, z_out, layer_type="conv",
                        apply_relu=False, apply_maxpool=False
                    )
                _evaluate_step(q_pre_act, s_out, z_out, f"features_{conv_idx}_pre_act")
                
                if mode == "int32":
                    if activation == "gelu":
                        q_x = quant_utils.integer_gelu_lut(q_pre_act, layer_data["gelu_lut"], z_out)
                    elif activation == "leaky_relu":
                        q_x = q_pre_act
                    else:
                        q_x = quant_utils.quantized_relu(q_pre_act, z_out)
                else:
                    q_x = quant_utils.quantized_relu(q_pre_act, z_out)
                    
                _evaluate_step(q_x, s_out, z_out, _activation_label(f"features_{conv_idx}", activation))

                if conv_idx in POOL_AFTER_CONV:
                    q_x = quant_utils.integer_max_pool2d(q_x, kernel_size=2, stride=2)

            if mode == "int32":
                q_pooled = quant_utils.integer_adaptive_max_pool2d(q_x, (7, 7))
            else:
                fc_in_scale = quant_state["classifier_0"]["scale_in"]
                fc_in_zp = quant_state["classifier_0"]["zp_in"]
                q_pooled = quant_utils.integer_adaptive_avg_pool(q_x, z_out, s_out, fc_in_zp, fc_in_scale, output_size=(7, 7))
            ...
            for fc_idx in FC_INDICES:
                ...
                layer_data = quant_state[layer_name]
                if mode == "int32" and not is_last and activation == "gelu":
                    layer_data["gelu_lut"] = quant_state[f"{layer_name}_gelu_lut"]
                
                if mode == "int32":
                    q_fc_pre, s_out, z_out = infer_module.run_integer_layer(
                        q_fc_in, layer_data, z_out, layer_type="linear",
                        apply_relu=False, apply_maxpool=False, activation=activation
                    )
                else:
                    q_fc_pre, s_out, z_out = infer_module.run_integer_layer(
                ...
                if is_last:
                    q_fc_in = q_fc_pre
                    _evaluate_step(q_fc_in, s_out, z_out, f"classifier_{fc_idx}")
                else:
                    _evaluate_step(q_fc_pre, s_out, z_out, f"classifier_{fc_idx}_pre_act")
                    if mode == "int32":
                        if activation == "gelu":
                            q_fc_in = quant_utils.integer_gelu_lut(q_fc_pre, layer_data["gelu_lut"], z_out)
                        elif activation == "leaky_relu":
                            q_fc_in = q_fc_pre
                        else:
                            q_fc_in = quant_utils.quantized_relu(q_fc_pre, z_out)
                    else:
                        q_fc_in = quant_utils.quantized_relu(q_fc_pre, z_out)
                    _evaluate_step(q_fc_in, s_out, z_out, _activation_label(f"classifier_{fc_idx}", activation))
...
def plot_error_metrics(metrics_dict, dataset_name, activation: str = "relu", mode: str = "int8"):
    ...
    fig.suptitle(
        f"VGG19 FP32 vs {mode.upper()} Quantization Error Progression ({dataset_name})",
        fontsize=16,
        fontweight="bold",
    )
    ...
    graphs_dir = os.path.join(THIS_DIR, "error", "graphs", safe_name)
...
if __name__ == "__main__":
    ...
    for dataset_name in selected:
        ...
        safe_name = dataset_name.lower().replace(" ", "_").replace("-", "_")
        results_dir = os.path.join(THIS_DIR, "error", "json", safe_name)
